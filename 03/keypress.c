...
#define SDL_WINDOW_WIDTH 620
#define SDL_WINDOW_HEIGHT 460
...
SDL_Surface *load_resource(const char *path) {
...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]) {
  ...
  for (size_t i = 0; i < SDL_arraysize(keypress_surface_bmp); i++) {
    SDL_Surface *surface = load_resource(keypress_surface_bmp[i].path);
    if (!surface) {
      return SDL_APP_FAILURE;
    }
